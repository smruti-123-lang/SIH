...
```bash
git clone https://github.com/smruti-123-lang/SIH
...
1. `train_model.py` loads `crop_yield.csv`, builds a Scikit-learn `Pipeline` with a `ColumnTransformer` (OneHotEncoder for categoricals, passthrough for numerics), fits a `LinearRegression` model, and serializes the full pipeline to `.pkl`.
2. `app.py` loads the saved pipeline on startup and exposes a `/predict` POST endpoint. Incoming JSON is converted to a DataFrame, columns are aligned to the training feature order (missing columns default to `0`), and the pipeline returns a yield prediction.

---

## 🏆 Achievement

Submitted to **Smart India Hackathon 2025** — qualified **Round 1**, ranking among the **top ~300 teams out of 100,000+ national entries**.
